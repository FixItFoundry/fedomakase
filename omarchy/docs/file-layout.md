...
Two Fedora packages are built from this one repo (spec files live in
...
`omarchy-keyring` (GPG keys for dnf) and `omarchy-nvim` (the Neovim
setup; independently seeds `/etc/skel`).

Three layers populate `$HOME`:

1. **Seed** — `omarchy-settings` ships static defaults to `/etc/skel/`.
   Fedora's `useradd -m` copies that tree into a new user's `$HOME` at user
...
`os-release`) are owned by upstream Fedora packages, so we can't install over
them via dnf without a file conflict. Instead they ship at
...
upgrade. This is documented in the spec file.
...
watcher cannot tell a bypassed `dnf upgrade` from the package transaction inside
...
`omarchy-migrate` waits for any active dnf transaction to finish, then runs
...
- `install/post-install/all.sh` — final dnf/udev/localdb passes.
...
| `/etc/` file owned by an upstream package | `default/`, then add to `etc-overrides` in `omarchy-settings` spec file + scriptlet |
| Package-owned system file (e.g. systemd user service/path in `/usr/lib`) | `default/`, document the mapping in `default/package-defaults.tsv`, then add the `install -Dm644` line in `omarchy-settings` spec file |
| Per-user file that's static but lives outside `~/.config` | `default/`, then add `install -Dm644 ... $pkgdir/etc/skel/...` in `omarchy-settings` spec file |
