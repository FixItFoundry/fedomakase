...
used when a package update needs to change state that dnf cannot safely own by
itself.
...
bypassed the dnf guard with `sudo env OMARCHY_ALLOW_DIRECT_DNF=1 dnf
-y`, and any second user on the machine, whose migration markers are per-user
